...
function createManager() {
	...
	manager.active_connection = Ti.App.Properties.getObject('active_connection', null);
	...
	if (manager.active_connection.type == "freebox") {
		...
		manager.api = require('titanium-freebox-os-client/freebox-os-client')({
			url: manager.active_connection.url,
		        	...
		        	if (response.success) {
						...
						for (var i = 0; i < connections.length; i++) {
							...
							if (c.type == "freebox" && c.uid == self.active_connection.uid) {
								...
								Ti.App.Properties.setObject('active_connection', self.active_connection);
								break;	
							}
						}
	...
	manager.addTorrent = function(options, callback) {
		...
		if (this.isFreebox) {
			...
			if (!_.isEmpty(options.url))
				...
				var torrent_file = Ti.Filesystem.getFile(Ti.Filesystem.tempDirectory, Ti.Platform.createUUID() + '.torrent');
				torrent_file.write(options.data);
				bodyParam.formData.download_file = torrent_file;
